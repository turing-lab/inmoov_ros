...
class Routine(object):
    ...
    def definitions(self):
        ...
        while not rospy.is_shutdown():
            for i in range (0,50):
                self.moveTo("l_shoulder_lift_joint",(i*1.1)+30)
                self.moveTo("l_shoulder_out_joint",(i*1.1)+80)
                self.moveTo("l_upper_arm_roll_joint",i+90)
                self.moveTo("l_elbow_flex_joint",i+40)
                sleep(0.05)
            for i in range (1,0,-1):
                for j in range(0,90):
                    self.moveTo("l_wrist_roll_joint",j)
                    sleep(0.04)
